...
public final class MachineController {
  ...
  public void start() {
    ...
    // 실행
    final Changes changes = buyItems(machine);
  ...
  private Changes buyItems(final Machine machine) {
    ...
    while (machine.canBuyAny()) {
      // 구매할 아이템명 입력
      final String itemName = inputView.inputItemName(machine.getRemainedAmount());
